...
using WoWonder.Activities.Chat.GroupChat.Fragment;
...
using MessageData = WoWonder.Helpers.Chat.MessageDataExtra;
...
namespace WoWonder.Activities.Chat.GroupChat
{
    ...
    public class GroupChatWindowActivity : AppCompatActivity, MaterialDialog.IListCallback, View.IOnLayoutChangeListener, IOnRecordClickListener, IOnRecordListener, SwipeReply.ISwipeControllerActions
    {
        ...
        private CircleImageView GroupChatProfile;
        public ImageView ChatMediaButton;
        public RecyclerView MRecycler;
        private RecyclerView RecyclerHiSuggestion;
        private GroupChatRecordSoundFragment ChatRecordSoundBoxFragment;
        ...
        private string GifFile, PermissionsType, ShowEmpty;
        private Toolbar ToolBar;
        private Timer Timer;
        private bool IsRecording;
        public ChatObject GroupData;
        public string GroupId, ChatId, ReplyId = "0";
        ...
        private void InitComponent()
        {
            try
            {
                ...
                ChatRecordSoundBoxFragment = new GroupChatRecordSoundFragment();
                ...
                GroupChatProfile = FindViewById<CircleImageView>(Resource.Id.userProfileImage);
                ...
                ActionBarSubTitle.Text = GetString(Resource.String.Lbl_Group_Chat_Description);
                ...
                Methods.SetColorEditText(EmojIconEditTextView, WoWonderTools.IsTabDark() ? Color.White : Color.Black);
                //ChatEmojImage.SetColorFilter(WoWonderTools.IsTabDark() ? Color.White : Color.ParseColor("#444444"));
                ...
                EmojisViewTools.LoadView(this, EmojIconEditTextView, "GroupChatWindowActivity", ChatEmojImage);
        ...
        private void AddOrRemoveEvent(bool addEvent)
        {
            try
            {
                ...
                if (addEvent)
                {
                    ...
                    GroupChatProfile.Click += GroupChatProfileOnClick;
                    ActionBarTitle.Click += GroupChatProfileOnClick;
                    ActionBarSubTitle.Click += GroupChatProfileOnClick;
        ...
        private void SuggestionAdapterOnItemClick(object sender, AdapterClickEvents e)
        {
            try
            {
                ...
                if (Methods.CheckConnectivity())
                {
                    ...
                    {
                        UserDetails.Socket?.EmitAsync_SendGroupMessage(GroupId, UserDetails.AccessToken, UserDetails.Username, SuggestionAdapter.GetItem(position).RealMessage, "", time2);
                    }
                    else
                    {
                        Task.Factory.StartNew(() =>
                        {
                            GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, SuggestionAdapter.GetItem(position).RealMessage).ConfigureAwait(false);
                        });
                    }
                }
                else
        ...
        private void GroupChatProfileOnClick(object sender, EventArgs e)
        {
            try
            {
        ...
        private void MoreButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (GroupData.Owner != null && GroupData.Owner.Value)
                    arrayAdapter.Add(GetText(Resource.String.Lbl_AddMembers));
        ...
        private void EditTextOpen()
        {
            try
            {
                ...
                {
                    RequestsAsync.Message.SetChatTypingStatusAsync(GroupId, "typing").ConfigureAwait(false);
                }
            }
            catch (Exception exception)
            {
            ...
            try
            {
                ...
                if (RecordButton?.Tag?.ToString() == "Audio")
                {
                    ...
                    if (!string.IsNullOrEmpty(filePath))
                    {
                        ...
                        //Here on This function will send Selected audio file to the Group 
                        if (Methods.CheckConnectivity())
                        {
                            Task.Factory.StartNew(() =>
                            {
                                GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, "", "", filePath, "", "", "", "", "", ReplyId).ConfigureAwait(false);
                            });
                        }
                        else
                        {
                ...
                else if (RecordButton?.Tag?.ToString() == "Text")
                {
                    ...
                    else
                    {
                        ...
                        SayHiLayout.Visibility = ViewStates.Gone;
                        SayHiSuggestionsRecycler.Visibility = ViewStates.Gone;
                        //Here on This function will send Text Messages to the Group 
                        ...
                        if (Methods.CheckConnectivity())
                        {
                            if (AppSettings.ConnectionTypeChat == InitializeWoWonder.ConnectionType.Socket)
                            {
                                UserDetails.Socket?.EmitAsync_SendGroupMessage(GroupId, UserDetails.AccessToken, UserDetails.Username, EmojIconEditTextView.Text, ReplyId, time2);
                            }
                            else
                            {
                                Task.Factory.StartNew(() =>
                                {
                                    GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, EmojIconEditTextView.Text, "", "", "", "", "", "", "", ReplyId).ConfigureAwait(false);
                                });
                            }
                        }
                        else
                ...
                if (requestCode == 506 && resultCode == Result.Ok) // right_contact
                {
                    ...
                    if (contact != null)
                    {
                        ...
                        if (Methods.CheckConnectivity())
                        {
                            ...
                            await Task.Factory.StartNew(() =>
                            {
                                GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, dataContact, "1", "", "", "", "", "", "", ReplyId).ConfigureAwait(false);
                            });
                        }
                        else
                        {
                        ...
                        if (result.IsSuccessful)
                        {
                            ...
                            if (!string.IsNullOrEmpty(resultUri.Path))
                            {
                                ...
                                if (Methods.CheckConnectivity())
                                {
                                    ...
                                    await Task.Factory.StartNew(() =>
                                    {
                                        GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, "", "", resultUri.Path, "", "", "", "", "", ReplyId).ConfigureAwait(false);
                                    });
                                }
                                else
                                {
                    ...
                    else
                    {
                        ...
                        if (Methods.MultiMedia.CheckFileIfExits(IntentController.CurrentPhotoPath) != "File Dont Exists")
                        {
                            ...
                            if (Methods.CheckConnectivity())
                            {
                                ...
                                await Task.Factory.StartNew(() =>
                                {
                                    GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, "", "", IntentController.CurrentPhotoPath, "", "", "", "", "", ReplyId).ConfigureAwait(false);
                                });
                            }
                            else
                            {
                    ...
                    if (Methods.MultiMedia.CheckFileIfExits(IntentController.CurrentVideoPath) != "File Dont Exists" && Build.VERSION.SdkInt <= BuildVersionCodes.OMr1)
                    {
                        ...
                        else
                        {
                            ...
                            if (Methods.CheckConnectivity())
                            {
                                ...
                                await Task.Factory.StartNew(() =>
                                {
                                    GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, "", "", IntentController.CurrentVideoPath, "", "", "", "", "", ReplyId).ConfigureAwait(false);
                                });
                            }
                            else
                            {
                ...
                else if (requestCode == 300 && resultCode == Result.Ok) // right_gif
                {
                    ...
                    if (gifLink != "Data not available" && !string.IsNullOrEmpty(gifLink))
                    {
                        ...
                        if (Methods.CheckConnectivity())
                        {
                            await Task.Factory.StartNew(() =>
                            {
                                GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, "", "", "", "", "", gifUrl, "", "", ReplyId).ConfigureAwait(false);
                            });
                        }
                        else
                        {
                    ...
                    if (!string.IsNullOrEmpty(placeLatLng))
                    {
                        ...
                        if (latLng?.Length > 0)
                        {
                            ...
                            if (Methods.CheckConnectivity())
                            {
                                await Task.Factory.StartNew(() =>
                                {
                                    GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, "", "", "", "", "", "", lat, lng, ReplyId).ConfigureAwait(false);
                                });
                            }
                            else
                            {
                    ...
                    if (!string.IsNullOrEmpty(videoPath))
                    {
                        try
                        {
                            ...
                            if (Methods.CheckConnectivity())
                            {
                                ...
                                await Task.Factory.StartNew(() =>
                                {
                                    GroupMessageController.SendMessageTask(this, GroupId, ChatId, time2, "", "", videoPath, "", "", "", "", "", ReplyId).ConfigureAwait(false);
                                });
                            }
                            else
                            {
        ...
        public void OnSelection(MaterialDialog dialog, View itemView, int position, string itemString)
        {
            try
            {
                ...
                else if (itemString == GetText(Resource.String.Lbl_GroupInfo))
                {
                    Intent intent = new Intent(this, typeof(EditGroupChatActivity));
                    intent.PutExtra("GroupObject", JsonConvert.SerializeObject(GroupData));
                    intent.PutExtra("Type", "Profile");
                ...
                else if (itemString == GetText(Resource.String.Lbl_AddMembers))
                {
                    Intent intent = new Intent(this, typeof(EditGroupChatActivity));
                    intent.PutExtra("GroupObject", JsonConvert.SerializeObject(GroupData));
                ...
                else if (itemString == GetText(Resource.String.Lbl_ExitGroup))
                {
                    if (!Methods.CheckConnectivity())
                    {
                        ToastUtils.ShowToast(this, GetString(Resource.String.Lbl_CheckYourInternetConnection), ToastLength.Short);
                    ...
                    {
                        var dialogBuilder = new MaterialDialog.Builder(this).Theme(WoWonderTools.IsTabDark() ? MaterialDialogsTheme.Dark : MaterialDialogsTheme.Light);
                        dialogBuilder.Content(GetText(Resource.String.Lbl_AreYouSureExitGroup));
                        dialogBuilder.PositiveText(GetText(Resource.String.Lbl_Exit)).OnPositive(async (materialDialog, action) =>
                        {
                            try
                            {
                                //Show a progress
                        ...
                        dialogBuilder.NegativeText(GetText(Resource.String.Lbl_Cancel)).OnNegative(new WoWonderTools.MyMaterialDialog());
        ...
        private void LoadData_Item()
        {
            try
            {
                GlideImageLoader.LoadImage(this, GroupData.Avatar, GroupChatProfile, ImageStyle.CircleCrop, ImagePlaceholders.Drawable);
                ...
                //100 people join this

                if (GroupData.Parts != null)
                    ActionBarSubTitle.Text = Methods.FunString.FormatPriceValue(GroupData.Parts.Count) + " " + GetText(Resource.String.Lbl_PeopleJoinThis);
                                ...
                                if (respondList > 0)
                                {
                                    ...
                                    foreach (var item in result.Data.Messages)
                                    {
                                        ...
                                        else if (check.MesData.Seen == "0" && check.MesData.Seen != item.Seen)
                                        {
                                            ...
                                            check.MesData = WoWonderTools.MessageFilter(GroupId, item, type, true);
                                            check.TypeView = type;
        ...
        private async Task LoadMoreMessages_API()
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    var (apiStatus, respond) = await RequestsAsync.GroupChat.FetchGroupChatMessagesAsync(GroupId, firstMessageId);
                    if (apiStatus == 200)
                    {
                        if (respond is GroupMessagesObject result)
                        {
        ...
        public void ReplyItems()
        {
            try
            {
                if (SelectedItemPositions != null)
                {
                    ...
                    //TxtOwnerName.Text = SelectedItemPositions.MesData.MessageUser?.User?.GroupId == UserDetails.UserId ? GetText(Resource.String.Lbl_You) : ActionBarTitle.Text;
        ...
        private void GetWallpaper()
        {
            try
            {
                ...
                {
                    RootView.Background = WoWonderTools.IsTabDark() ? new ColorDrawable(Color.ParseColor("#282828")) : new ColorDrawable(Color.ParseColor("#F8F8F7"));
                }
            }
            catch (Exception e)
            {
        ...
        private void OpenDialogGallery(string typeImage)
        {
            try
            {
                ...
                if ((int)Build.VERSION.SdkInt < 23)
                {
                    ...
                    //Open Image 
                    var myUri = Uri.FromFile(new File(Methods.Path.FolderDiskImage + "/" + GroupId, Methods.GetTimestamp(DateTime.Now) + ".jpg"));
                    CropImage.Activity()
        ...
        {
            private readonly GroupChatWindowActivity ChatWindow;
            public MyTextWatcher(GroupChatWindowActivity chatWindow)
            {
