...
namespace WoWonder.Activities.Tabbes
{
    ...
    public class TabbedMainActivity : AppCompatActivity, MaterialDialog.IListCallback 
    {
        ...
        private RelativeLayout RlSearch, RlMessage, RlReels, RlAdd;
        ...
        public override void OnConfigurationChanged(Configuration newConfig)
        {
            try
            {
                ...
                {
                    MeoNavigationTabBar?.SetBackgroundBottomColor(WoWonderTools.IsTabDark() ? Color.Black : Color.White);
                    MeoNavigationTabBar?.SetCircleColor(WoWonderTools.IsTabDark() ? Color.Black : Color.White);
                }   
            }
            catch (Exception e)
            {
        ...
        private void InitComponent()
        {
            try
            {
                ...
                RlReels = FindViewById<RelativeLayout>(Resource.Id.rlReels);
        ...
        private void ShowPopup(View v)
        {
            try
            {
                ...
                int px = (int)TypedValue.ApplyDimension(ComplexUnitType.Dip, 180, this.Resources.DisplayMetrics);
            ...
            { 
                ...
                { 
                    ...
                    if (Methods.CheckConnectivity())
                        PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => ApiRequest.Get_MyProfileData_Api(this), ApiRequest.GetTimeZoneAsync, ApiRequest.GetMyGroups, ApiRequest.GetMyPages, ApiRequest.GetLastArticles, ApiRequest.LoadSuggestedUser, ApiRequest.LoadSuggestedGroup, ApiRequest.LoadSuggestedPage, ApiPostAsync.GetAllPostVideo });
        ...
        public void GetOneSignalNotification()
        {
            try
            {
                ...
                if (notifier == "Chat" || notifier == "Notifier")
                {
                    Intent intent;
                    if (type == "user")
                    {
                        var item = MsgTabbedMainActivity.GetInstance()?.LastChatTab?.MAdapter?.LastChatsList?.FirstOrDefault(a => a.LastChat?.UserId == userId && a.LastChat?.ChatType == "user");
                        ...
                        {
                            if (item.LastChat.LastMessage.LastMessageClass != null) mainChatColor = item.LastChat.LastMessage.LastMessageClass.ChatColor.Contains("rgb") ? Methods.FunString.ConvertColorRgBtoHex(item.LastChat.LastMessage.LastMessageClass.ChatColor) : item.LastChat.LastMessage.LastMessageClass.ChatColor ?? AppSettings.MainColor;
                    ...
                    else if (type == "page")
                    {
                        ...
                        if (AppSettings.LastChatSystem == SystemGetLastChat.Default)
                            item = MsgTabbedMainActivity.GetInstance()?.LastChatTab?.MAdapter?.LastChatsList?.FirstOrDefault(a => a.LastChat?.PageId == pageId && a.LastChat?.ChatType == "page");
                        else
                            item = MsgTabbedMainActivity.GetInstance()?.LastPageChatsTab?.MAdapter?.LastChatsList?.FirstOrDefault(a => a.LastChatPage?.PageId == pageId);
                    ...
                    else if (type == "group")
                    {
                        Classes.LastChatsClass item;
                        if (AppSettings.LastChatSystem == SystemGetLastChat.Default)
                            item = MsgTabbedMainActivity.GetInstance()?.LastChatTab?.MAdapter?.LastChatsList?.FirstOrDefault(a => a.LastChat?.GroupId == groupId && a.LastChat?.ChatType == "group");
                        else
                            item = MsgTabbedMainActivity.GetInstance()?.LastGroupChatsTab?.MAdapter?.LastChatsList?.FirstOrDefault(a => a.LastChat?.GroupId == groupId);
                    ...
                    else if (type is "call_audio" or "call_video")
                    {
                        if (Methods.CheckConnectivity())
