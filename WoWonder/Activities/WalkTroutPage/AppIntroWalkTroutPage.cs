        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                if (Methods.CheckConnectivity())
                    PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => AppUpdaterHelper.FetchFirstNewsFeedApiPosts(true), ApiRequest.LoadSuggestedUser, ApiRequest.LoadSuggestedGroup, ApiRequest.LoadSuggestedPage, ApiRequest.GetMyGroups, ApiRequest.GetMyPages });
            }
            catch (Exception e)
            {
            ...
            try
            {
                if (Caller.Contains("register"))
                {
                    switch (ListUtils.SettingsSiteList?.MembershipSystem)
                    {
                        ...
                        default:
                            { 
                                switch (AppSettings.AddAllInfoPorfileAfterRegister)
                                {
                                    case true:
                                        {
