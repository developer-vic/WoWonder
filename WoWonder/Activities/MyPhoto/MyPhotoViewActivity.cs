...
namespace WoWonder.Activities.MyPhoto
{
    ...
    public class MyPhotoViewActivity : AppCompatActivity
    {
        ...
        private ObservableCollection<PostDataObject> PostList;
        ...
        private void Download_OnClick()
        {
            try
            {
                ...
                { 
                    ...
                    var photos = !string.IsNullOrEmpty(PostList[IndexImage].PostSticker) ? PostList[IndexImage].PostSticker : PostList[IndexImage].PostFileFull;
                     
                    Methods.CapturePhotoUtils.InsertImage(ContentResolver, photos);
        ...
        private void Get_DataImage()
        {
            try
            {
                ...
                PostList = JsonConvert.DeserializeObject<ObservableCollection<PostDataObject>>(Intent?.GetStringExtra("PostList") ?? "");
                if (PostData != null)
                {
                    ViewPager.Adapter = new TouchMyPhotoAdapter(this, PostList);
        ...
        private void ViewPagerOnPageScrolled(object sender, ViewPager.PageScrolledEventArgs e)
        {
            try
            {
                switch (e.Position)
                {
                    case >= 0 when PostList.Count > e.Position:
                    {
                        PostData = PostList[e.Position];
                        if (PostData != null)
                            SetDataPost();
                        break;
                    }
