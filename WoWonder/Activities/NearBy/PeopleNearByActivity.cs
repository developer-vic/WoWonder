                        ...
                        case true:
                            ...
                            RunOnUiThread(() =>
                            {
                                try
                                {
                                    ...
                                    dialog.Title(GetString(Resource.String.Lbl3_Use_Location) + "?").TitleColorRes(Resource.Color.primary);
                                    dialog.Content(GetString(Resource.String.Lbl3_GPS_is_disabled) + "?");
                                    dialog.PositiveText(GetString(Resource.String.Lbl_Ok)).OnPositive((materialDialog, action) =>
                                    {
                                        //Open intent Gps
                                        new IntentController(this).OpenIntentGps(LocationManager);
                                    });
                                    dialog.NegativeText(GetString(Resource.String.Lbl_Cancel)).OnNegative(new WoWonderTools.MyMaterialDialog());
