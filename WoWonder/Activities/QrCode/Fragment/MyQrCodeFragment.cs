    ...
    public class MyQrCodeFragment : AndroidX.Fragment.App.Fragment
    {
        ...
        private void BtnSaveOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                Methods.CapturePhotoUtils.InsertImage(Activity.ContentResolver, path);
        ...
        private async void BtnShareOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                var path = Methods.MultiMedia.Export_Bitmap_As_Image(BitmapImage, "QrCode_Image", Methods.Path.FolderDiskImage);
                //path = Methods.CapturePhotoUtils.InsertImage(Activity.ContentResolver, path);

                await ShareFileImplementation.ShareRemoteFile(Activity, UrlUserData, path, "QrCode_Image", Context.GetText(Resource.String.Lbl_Send_to));
            }
            catch (Exception exception)
            {
                Methods.DisplayReportResultTrack(exception);
        ...
        private void Get_Data_User()
        {
            ...
            { 
                ...
                if (local != null)
                {
                    ...
                    GlideImageLoader.LoadImage(Activity, local.Avatar, ProfileAvatarImage, ImageStyle.CircleCrop, ImagePlaceholders.Drawable);
